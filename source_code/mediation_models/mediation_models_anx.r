...
#
#################################################################
# with anxiety score instead of depression score at W11
#################################################################
#
...
sink("data/analysis/mediation_output_anx.txt")
...
model_general <- "
  ...
  anxiety_score_ord ~ b1*maladaptive_score + b2*adaptive_score_box
...
p_values_general <- parameterEstimates(fit_general) |>
  filter((lhs == "anxiety_score_ord" | lhs == "maladaptive_score" | lhs == "adaptive_score_box" | 
...
model_sadness <- "
  ...
  sadness_adaptive_score_ord ~ a7*extraversion + a8*agreeableness + 
                               a9*conscientiousness + a10*neuroticism +
                               a11*openness + a12*sex
  anxiety_score_ord ~ b1*sadness_maladaptive_score + b2*sadness_adaptive_score_ord
...
p_values_sadness <- parameterEstimates(fit_sadness) |>
  filter((lhs == "anxiety_score_ord" | lhs == "sadness_maladaptive_score" | lhs == "sadness_adaptive_score_ord" | 
...
p_values_anxiety <- parameterEstimates(fit_anxiety) |>
  filter((lhs == "anxiety_score_ord" | lhs == "anxiety_maladaptive_score" | lhs == "anxiety_adaptive_score_box" | 
...
model_anger <- "
  ...
  anger_adaptive_score_yeo ~ a7*extraversion + a8*agreeableness + 
                               a9*conscientiousness + a10*neuroticism +
                               a11*openness + a12*sex
  anxiety_score_ord ~ b1*anger_maladaptive_score + b2*anger_adaptive_score_yeo
...
p_values_anger <- parameterEstimates(fit_anger) |>
  filter((lhs == "anxiety_score_ord" | lhs == "anger_maladaptive_score" | lhs == "anger_adaptive_score_yeo" | 
...
write_csv(adjusted_p_values, "data/analysis/mediation_adjusted_p_values_anx.csv")
