...
data_general <- read_csv("data/analysis/no_outliers_data_med_gen_cbcl.csv")
data_sadness <- read_csv("data/analysis/no_outliers_data_med_sad_cbcl.csv")
data_anxiety <- read_csv("data/analysis/no_outliers_data_med_anx_cbcl.csv")
data_anger <- read_csv("data/analysis/no_outliers_data_med_ang_cbcl.csv")

# Set sink to capture all output in a text file ----
sink("data/analysis/no_outliers_mediation_output_cbcl.txt")
...
model_general <- "
  ...
  depression_score_yeo ~ c1*extraversion + c2*agreeableness + 
                         ...
                         c5*openness + c6*internalizing + c7*sex
  
  # Mediator
  maladaptive_score_yeo ~ a1*extraversion + a2*agreeableness + 
                      a3*conscientiousness + a4*neuroticism +
                      a5*openness + a6*internalizing + a7*sex
  adaptive_score_yeo ~ a8*extraversion + a9*agreeableness + 
                       a10*conscientiousness + a11*neuroticism +
                       a12*openness + a13*internalizing + a14*sex
  depression_score_yeo ~ b1*maladaptive_score_yeo + b2*adaptive_score_yeo

  # Covariance between mediators
  maladaptive_score_yeo ~~ adaptive_score_yeo
  ...
  # Predictor-specific total indirect effects
    ...
    ind_int := ind_int_mal + ind_int_ada
...
model_sadness <- "
  ...
  sadness_maladaptive_score ~ a1*extraversion + a2*agreeableness + 
                              a3*conscientiousness + a4*neuroticism +
                              a5*openness + a6*internalizing + a7*sex
  sadness_adaptive_score_box ~ a8*extraversion + a9*agreeableness + 
                               a10*conscientiousness + a11*neuroticism +
                               a12*openness + a13*internalizing + a14*sex
  depression_score_yeo ~ b1*sadness_maladaptive_score + b2*sadness_adaptive_score_box
  ...
  # Predictor-specific indirect effects via each mediator
    ...
    ind_int_sad_mal := a6*b1
    ind_sex_sad_mal := a7*b1

    ind_ext_sad_ada := a8*b2
    ind_agr_sad_ada := a9*b2
    ind_con_sad_ada := a10*b2
    ind_neu_sad_ada := a11*b2
    ind_ope_sad_ada := a12*b2
    ind_int_sad_ada := a13*b2
    ind_sex_sad_ada := a14*b2
  ...
  # Predictor-specific total indirect effects
    ...
    ind_int := ind_int_sad_mal + ind_int_sad_ada
...
model_anxiety <- "
  ...
  anxiety_maladaptive_score ~ a1*extraversion + a2*agreeableness + 
                              a3*conscientiousness + a4*neuroticism +
                              a5*openness + a6*internalizing + a7*sex
  anxiety_adaptive_score_box ~ a8*extraversion + a9*agreeableness + 
                               a10*conscientiousness + a11*neuroticism +
                               a12*openness + a13*internalizing + a14*sex
  ...
  # Predictor-specific indirect effects via each mediator
    ...
    ind_int_anx_mal := a6*b1
    ind_sex_anx_mal := a7*b1

    ind_ext_anx_ada := a8*b2
    ind_agr_anx_ada := a9*b2
    ind_con_anx_ada := a10*b2
    ind_neu_anx_ada := a11*b2
    ind_ope_anx_ada := a12*b2
    ind_int_anx_ada := a13*b2
    ind_sex_anx_ada := a14*b2
  ...
  # Predictor-specific total indirect effects
    ...
    ind_int := ind_int_anx_mal + ind_int_anx_ada
...
model_anger <- "
  ...
  # Predictor-specific indirect effects via each mediator
    ...
    ind_int_ang_mal := a6*b1
    ind_sex_ang_mal := a7*b1

    ind_ext_ang_ada := a8*b2
    ind_agr_ang_ada := a9*b2
    ind_con_ang_ada := a10*b2
    ind_neu_ang_ada := a11*b2
    ind_ope_ang_ada := a12*b2
    ind_int_ang_ada := a13*b2
    ind_sex_ang_ada := a14*b2
  ...
  # Predictor-specific total indirect effects
    ...
    ind_int := ind_int_ang_mal + ind_int_ang_ada
...
write_csv(adjusted_p_values, "data/analysis/no_outliers_mediation_adjusted_p_values_cbcl.csv")