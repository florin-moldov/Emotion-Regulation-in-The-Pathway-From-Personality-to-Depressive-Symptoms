# Mediation models per emotion regulation context with complete cases and bootstrap standard errors
...
data_general <- read_csv("data/analysis/no_outliers_complete_cases_data_med_gen.csv")
data_sadness <- read_csv("data/analysis/no_outliers_complete_cases_data_med_sad.csv")
data_anxiety <- read_csv("data/analysis/no_outliers_complete_cases_data_med_anx.csv")
data_anger <- read_csv("data/analysis/no_outliers_complete_cases_data_med_ang.csv")

# Set sink to capture all output in a text file ----
sink("data/analysis/no_outliers_complete_cases_mediation_output.txt")
...
model_general <- "
  ...
  maladaptive_score ~ a1*extraversion + a2*agreeableness + 
                      ...
                      a5*openness + a6*sex
  adaptive_score_yeo ~ a7*extraversion + a8*agreeableness + 
                       a9*conscientiousness + a10*neuroticism +
                       a11*openness + a12*sex
  depression_score_yeo ~ b1*maladaptive_score + b2*adaptive_score_yeo

  # Covariance between mediators
  maladaptive_score ~~ adaptive_score_yeo
...
fit_general <- sem(model_general, data = data_general,
...
p_values_general <- parameterEstimates(fit_general) |>
  filter((lhs == "depression_score_yeo" | lhs == "maladaptive_score" | lhs == "adaptive_score_yeo" | 
...
model_sadness <- "
  ...
  sadness_adaptive_score_box ~ a7*extraversion + a8*agreeableness + 
                               a9*conscientiousness + a10*neuroticism +
                               a11*openness + a12*sex
  depression_score_sqrt ~ b1*sadness_maladaptive_score + b2*sadness_adaptive_score_box
...
fit_sadness <- sem(model_sadness, data = data_sadness,
              se = "bootstrap", bootstrap = 10000)
...
p_values_sadness <- parameterEstimates(fit_sadness) |>
  filter((lhs == "depression_score_sqrt" | lhs == "sadness_maladaptive_score" | lhs == "sadness_adaptive_score_box" | 
...
fit_anxiety <- sem(model_anxiety, data = data_anxiety,
              se = "bootstrap", bootstrap = 10000)
...
fit_anger <- sem(model_anger, data = data_anger,
              se = "bootstrap", bootstrap = 10000)
...
write_csv(adjusted_p_values, "data/analysis/no_outliers_complete_cases_mediation_adjusted_p_values.csv")