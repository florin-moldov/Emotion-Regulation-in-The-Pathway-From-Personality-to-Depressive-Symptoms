...
# Controlling for internalizing problems at W6
#################################################################
...
sink("data/analysis/no_outliers_lms_cbcl_output.txt")

# Define and fit the model for hypothesis 1 ----
# Filter out IDs with outliers 
outlier_ids <- read_csv("data/analysis/h1_outliers_cbcl.csv") |> 
               pull(ID)
...
fit_h1_check <- lm(depression_score ~ extraversion + agreeableness +
                     ...
                     internalizing + sex, data = data_h1_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h1_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_dep)
# Yeo-Johnson transformation with lambda = -1.409483 is the best transformation
...
fit_h1_check_transformed <- lm(depression_score_yeo ~ extraversion +
                                 ...
                                 conscientiousness + neuroticism + openness +
                                 internalizing + sex, data = data_h1_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h1_transformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h1_check_transformed)) # looks better
dev.off()
print(check_normality(fit_h1_check_transformed)) # still not OK but better
...
model_h1 <- "
  # Depression (depression_score_yeo) ~ personality + internalizing + sex
  depression_score_yeo ~ extraversion + agreeableness + 
                         conscientiousness + neuroticism + openness + internalizing + sex
"
...
# Filter out IDs with outliers 
outlier_ids <- read_csv("data/analysis/h2_general_outliers_cbcl.csv") |> 
               pull(ID)
...
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_general_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_dep)
# Sqrt(x+a) transformation with a = 0 is the best transformation
# Initialize and assign with length-matched predictions
data_h2_gen_no_outliers$depression_score_sqrt <- NA_real_
data_h2_gen_no_outliers$depression_score_sqrt[non_missing_idx] <- as.numeric(
  predict(bn_dep, newdata = data_h2_gen_no_outliers$depression_score[non_missing_idx])
)

# Check model assumptions again
fit_h2_general_check_transformed <- lm(depression_score_sqrt ~
                                         maladaptive_score +
                                           adaptive_score +
                                           internalizing +
                                           sex, data = data_h2_gen_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_general_transformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h2_general_check_transformed)) # looks better
dev.off()
print(check_normality(fit_h2_general_check_transformed)) # still not OK but better
print(check_heteroscedasticity(fit_h2_general_check_transformed)) # now OK

# Now we can fit the lavaan model with the transformed outcome variable
# and bootstrap confidence intervals
model_h2_general <- "
  # Depression (depression_score_sqrt) ~ general emotion regulation + internalizing + sex
  depression_score_sqrt ~ maladaptive_score + adaptive_score + internalizing + sex
"
...
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h2_sadness_outliers_cbcl.csv") |> 
               pull(ID)
...
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_sadness_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_dep)
# Yeo-Johnson with lambda = -1.398364 is the best transformation
# Initialize and assign with length-matched predictions
data_h2_sad_no_outliers$depression_score_yeo <- NA_real_
data_h2_sad_no_outliers$depression_score_yeo[non_missing_idx] <- as.numeric(
...
fit_h2_sadness_check_transformed <- lm(depression_score_yeo ~
                                         sadness_maladaptive_score +
                                           ...
                                           internalizing +
                                           sex, data = data_h2_sad_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_sadness_transformed_with_cbcl.svg", width = 12, height = 10)
...
model_h2_sadness <- "
  ...
  depression_score_yeo ~ sadness_maladaptive_score + sadness_adaptive_score + internalizing + sex
"
...
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h2_anxiety_outliers_cbcl.csv") |> 
               pull(ID)
...
# Separate model for checking assumptions since lavaan's sem model 
# doesn't work with check_model() function from performance package
fit_h2_anxiety_check <- lm(depression_score ~ 
                              anxiety_maladaptive_score + 
                              anxiety_adaptive_score + 
                              internalizing + 
                              sex, data = data_h2_anx_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_anxiety_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_dep)
# Yeo-Johnson transformation with lambda = -1.409472 is the best transformation
# Initialize and assign with length-matched predictions
...
# Check model assumptions again
fit_h2_anxiety_check_transformed <- lm(depression_score_yeo ~ 
                                          anxiety_maladaptive_score + 
                                          anxiety_adaptive_score + 
                                          internalizing + 
                                          sex, data = data_h2_anx_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_anxiety_transformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h2_anxiety_check_transformed)) # looks better
dev.off()
print(check_normality(fit_h2_anxiety_check_transformed)) # still not OK but better
...
model_h2_anxiety <- '
  # Depression (depression_score_yeo) ~ anxiety-specific emotion regulation strategy use + internalizing + sex
...
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h2_anger_outliers_cbcl.csv") |> 
               pull(ID)
...
# Separate model for checking assumptions since lavaan's sem model 
# doesn't work with check_model() function from performance package
fit_h2_anger_check <- lm(depression_score ~ 
                            anger_maladaptive_score + 
                            anger_adaptive_score + 
                            internalizing + 
                            sex, data = data_h2_ang_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_anger_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_dep)
# Yeo-Johnson transformation with lambda = -1.386208 is the best transformation
# Initialize and assign with length-matched predictions
...
# Check model assumptions again
fit_h2_anger_check_transformed <- lm(depression_score_yeo ~ 
                                        anger_maladaptive_score + 
                                        anger_adaptive_score + 
                                        internalizing + 
                                        sex, data = data_h2_ang_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h2_anger_transformed_with_cbcl.svg", width = 12, height = 10)
...
# Now we can fit the lavaan model with the transformed outcome variable and bootstrap confidence intervals
model_h2_anger <- '
  # Depression (depression_score_yeo) ~ anger-specific emotion regulation strategy use + internalizing + sex
  depression_score_yeo ~ anger_maladaptive_score + anger_adaptive_score + internalizing + sex
'
...
# Emotion-general model ----
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h3_general_outliers_cbcl.csv") |> 
               pull(ID)
...
fit_h3_mal_gen_check <- lm(maladaptive_score ~ extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_gen_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_mal_general_untransformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h3_mal_gen_check)) # meh
dev.off()
print(check_normality(fit_h3_mal_gen_check)) # OK
print(check_heteroscedasticity(fit_h3_mal_gen_check)) # not OK

fit_h3_ad_gen_check <- lm(adaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_gen_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_general_untransformed_with_cbcl.svg", width = 12, height = 10)
...
# Given violation of homoscedasticity, determine best transformation
# method for maladaptive_score (outcome variable)
# We can use the bestNormalize package for this
non_missing_idx <- !is.na(data_h3_gen_no_outliers$maladaptive_score)
# Fit on non-missing values only
bn_mal <- bestNormalize(data_h3_gen_no_outliers$maladaptive_score[non_missing_idx], r = 100)
print(bn_mal)
# Yeo-Johnson transformation with lambda = 1.382981 is the best transformation
# Initialize and assign with length-matched predictions
data_h3_gen_no_outliers$maladaptive_score_yeo <- NA_real_
data_h3_gen_no_outliers$maladaptive_score_yeo[non_missing_idx] <- as.numeric(
  predict(bn_mal, newdata = data_h3_gen_no_outliers$maladaptive_score[non_missing_idx])
)

# Check again the assumptions with the transformed outcome variable
fit_h3_mal_gen_check_transformed <- lm(maladaptive_score_yeo ~ extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_gen_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_mal_general_transformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h3_mal_gen_check_transformed)) # looks better
dev.off()
print(check_normality(fit_h3_mal_gen_check_transformed)) # still OK
print(check_heteroscedasticity(fit_h3_mal_gen_check_transformed)) # now OK  
...
print(bn_ada)
# Yeo-Johnson transformation with lambda = 2.608531 is the best transformation
# Initialize and assign with length-matched predictions
...
fit_h3_ad_gen_check_transformed <- lm(adaptive_score_yeo ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_gen_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_general_transformed_with_cbcl.svg", width = 12, height = 10)
...
model_h3_general <- '
  # Maladaptive emotion regulation strategy use ~ personality + internalizing + sex
  # Adaptive emotion regulation strategy use ~ personality + internalizing + sex
    maladaptive_score_yeo ~ extraversion + agreeableness + conscientiousness + neuroticism + openness + internalizing + sex
    adaptive_score_yeo ~ extraversion + agreeableness + conscientiousness + neuroticism + openness + internalizing + sex
'
...
# Sadness-specific model ----
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h3_sadness_outliers_cbcl.csv") |> 
               pull(ID)
...
fit_h3_mal_sad_check <- lm(sadness_maladaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_sad_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_mal_sadness_untransformed_with_cbcl.svg", width = 12, height = 10)
...
fit_h3_ad_sad_check <- lm(sadness_adaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_sad_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_sadness_untransformed_with_cbcl.svg", width = 12, height = 10)
print(check_model(fit_h3_ad_sad_check)) # a bit ambiguous
dev.off()
print(check_normality(fit_h3_ad_sad_check)) # not OK
print(check_heteroscedasticity(fit_h3_ad_sad_check)) # not OK
...
print(bn_ada_sad)
# Box-Cox transformation with lambda = 1.999941 is the best transformation
# Initialize and assign with length-matched predictions
...
fit_h3_ad_sad_check_transformed <- lm(sadness_adaptive_score_box ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_sad_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_sadness_transformed_with_cbcl.svg", width = 12, height = 10)
...
# Now we can fit the multivariate lavaan model 
model_h3_sad <- '
  # Maladaptive sad emotion regulation strategy use ~ personality + internalizing + sex
  # Adaptive sad emotion regulation strategy use ~ personality + internalizing + sex
    sadness_maladaptive_score ~ extraversion + agreeableness + conscientiousness + neuroticism + openness + internalizing + sex
    sadness_adaptive_score_box ~ extraversion + agreeableness + conscientiousness + neuroticism + openness + internalizing + sex
'
...
# Anxiety-specific model ----
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h3_anxiety_outliers_cbcl.csv") |> 
               pull(ID)
...
fit_h3_mal_anx_check <- lm(anxiety_maladaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_anx_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_mal_anxiety_untransformed_with_cbcl.svg", width = 12, height = 10)
...
fit_h3_ad_anx_check <- lm(anxiety_adaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_anx_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_anxiety_untransformed_with_cbcl.svg", width = 12, height = 10)
...
print(bn_ada_anx)
# Box-Cox transformation with lambda = 1.999943 is the best transformation
# Initialize and assign with length-matched predictions
...
fit_h3_ad_anx_check_transformed <- lm(anxiety_adaptive_score_box ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_anx_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_anxiety_transformed_with_cbcl.svg", width = 12, height = 10)
...
# Now we can fit the multivariate lavaan model 
model_h3_anx <- '
  # Maladaptive anxiety emotion regulation strategy use ~ personality + internalizing + sex
  # Adaptive anxiety emotion regulation strategy use ~ personality + internalizing + sex
...
# Anger-specific model ----
# Filter out IDs with outliers
outlier_ids <- read_csv("data/analysis/h3_anger_outliers_cbcl.csv") |> 
               pull(ID)
...
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_mal_anger_untransformed_with_cbcl.svg", width = 12, height = 10)
...
fit_h3_ad_ang_check <- lm(anger_adaptive_score ~  extraversion + agreeableness + 
                   conscientiousness + neuroticism + openness + internalizing + sex, data = data_h3_ang_no_outliers)
svg("reports/plots/lm_diagnostics/no_outliers_check_model_h3_ad_anger_untransformed_with_cbcl.svg", width = 12, height = 10)
...
write_csv(adjusted_p_values, "data/analysis/no_outliers_lms_adjusted_p_values_with_cbcl.csv")
...
all_outlier_ids <- bind_rows(
  read_csv("data/analysis/h1_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h2_general_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h3_general_outliers_cbcl.csv", show_col_types = FALSE)
) |>
  mutate(ID = trimws(as.character(ID))) |>
  distinct(ID)
...
write_csv(data_med_gen, "data/analysis/no_outliers_data_med_gen_cbcl.csv")

# For sadness-specific emotion model ----
all_outlier_ids <- bind_rows(
  read_csv("data/analysis/h1_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h2_sadness_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h3_sadness_outliers_cbcl.csv", show_col_types = FALSE)
) |>
  mutate(ID = trimws(as.character(ID))) |>
  distinct(ID)
...
data_med_sad <- data_med_sad |>
  mutate(
    depression_score_yeo = data_h2_sad_no_outliers$depression_score_yeo[match(ID, data_h2_sad_no_outliers$ID)],
    sadness_adaptive_score_box = data_h3_sad_no_outliers$sadness_adaptive_score_box[match(ID, data_h3_sad_no_outliers$ID)]
  )
write_csv(data_med_sad, "data/analysis/no_outliers_data_med_sad_cbcl.csv")

# For anxiety-specific emotion model ----
all_outlier_ids <- bind_rows(
  read_csv("data/analysis/h1_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h2_anxiety_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h3_anxiety_outliers_cbcl.csv", show_col_types = FALSE)
) |>
  mutate(ID = trimws(as.character(ID))) |>
  distinct(ID)
...
write_csv(data_med_anx, "data/analysis/no_outliers_data_med_anx_cbcl.csv")

# For anger-specific emotion model ----
all_outlier_ids <- bind_rows(
  read_csv("data/analysis/h1_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h2_anger_outliers_cbcl.csv", show_col_types = FALSE),
  read_csv("data/analysis/h3_anger_outliers_cbcl.csv", show_col_types = FALSE)
) |>
  mutate(ID = trimws(as.character(ID))) |>
  distinct(ID)
